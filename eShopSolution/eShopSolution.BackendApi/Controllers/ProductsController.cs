...
namespace eShopSolution.BackendApi.Controllers
{
    ...
    public class ProductsController : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateImage(int productId, [FromForm] ProductImageCreateRequest request)
        {
            ...
            var iamge = await _manageProductService.GetImageById(imageId);

            //return Created(nameof(GetById), product);
            return CreatedAtAction(nameof(GetImageById), new { id = imageId }, iamge);
        }

        [HttpGet("{productId}/iamges/{imageId}")]
        public async Task<IActionResult> GetImageById(int productId, int imageId)
        {
            var image = await _manageProductService.GetImageById(imageId);
            if (image == null)
                return BadRequest("Not found iamge");
            return Ok(image);
        }

        [HttpPut("{productId}/iamges/{imageId}")]
        ...
        [HttpDelete("{productId}/iamges/{imageId}")]
